...
const FlipSkills = () => {
  // Refs & states for each skill group
  ...
  const skillGroups = [imgSec01, imgSec02, imgSec03, imgSec04, imgSec05];

  const stringRef = useRef(null);

  useEffect(() => {
    // Animation function for flipping images
    const startFlip = (ref, setIndex, images) => {
      return setInterval(() => {
        if (!ref.current) return; // avoid "target null" error
        gsap.fromTo(
          ref.current,
          { scale: 0, opacity: 0 },
    ...
    requestAnimationFrame(() => {
      const intervals = imgRefs.map((ref, i) =>
        startFlip(ref, indexes[i][1], skillGroups[i])
      );
  ...
  // String hover effect (only for desktop)
  useEffect(() => {
  ...
  return (
    <div className="row d-flex justify-content-center position-relative align-items-center">
      ...
      <div className="row StringRow position-absolute d-flex justify-content-center align-items-center">
        <div ref={stringRef} style={{ width: "100%", overflow: "hidden" }}>
      ...
      {/* Flipping Skill Icons */}
      <div className="flipRow col-12">
        {skillGroups.map((group, i) => (
          <div className="flipDiv" key={i}>
            <img
              ref={imgRefs[i]}
              src={group[indexes[i][0]]}
              alt="Flipping Skill"
            />
          </div>
        ))}
